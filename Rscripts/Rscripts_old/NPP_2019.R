...
# select columns from dendro 2020
dendro_2020 <- select(dendro_2020, week, subplot, tag, 
...
all_trees_weeks <- all_trees %>% 
  ...
  slice(rep(1:n(), each = 19)) %>% # this number must match # of data points 
  group_by(tag) %>% 
...
# also correct the 2018 & 2020 dates (they were interpretted as all the same year)
...
names(dates) <- c("date")
dates <- data.frame(recode_factor(dates$date, '2019-07-26' = "2020-07-26"))
names(dates) <- c("date")
dates <- data.frame(recode_factor(dates$date, '2019-07-29' = "2020-07-29"))
names(dates) <- c("date")
...
final_output <- matrix(nrow= 3421, ncol=4, dimnames = list(c(), 
                                                           c("uniqueID", "pvalue", 
                                                             "slope", "intercept")))
...
#remove clutter from global environment
remove(df_sub, df_summary, df.list, final_output,final_output_df,
